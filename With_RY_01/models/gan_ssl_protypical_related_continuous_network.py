...
class GAN_Protypical_Related_Continuous_Network():
    def __init__(self, classifier_options, discriminator_options, global_options, valid_0_dim,
                 fake_0_dim, tb_v):
        super(GAN_Protypical_Related_Continuous_Network, self).__init__()
    ...
    def train(self, sample, more_attention_D=True, more_attention_C=False):
        ...
        yu_logits = self.classifier.forward(sample, run_type=0)[2]
        predict_label_p = torch.exp(yu_logits)
        if self.global_options['cuda']:
            predict_label_p = predict_label_p.cuda()
        ...
        for i in range(k_d):
            ...
            # self.discriminator_optimizor.zero_grad()
            self.real_loss = self.adversarial_loss(self.discriminator(xq, yq_one_hot.view(*predict_label_p.size()[:]), input_type='query'), self.valid)
            self.fake_loss = self.adversarial_loss(self.discriminator(xu, predict_label_p.detach(), input_type='unlabeled'), self.fake)
            self.d_loss = (self.real_loss*0.35 + self.fake_loss*0.65) / 1
        ...
        for i in range(k_c):
            ...
            self.discriminator.calculate_prototypical(xs)
            fake_dis = self.discriminator(xu, predict_label_p, 'unlabeled')
