...
class ImageProcess:
    ...
    def __init__(self, gt_dir, s_dir, output_dir, format_size=[64, 64]):
        # initial parameter
        self.gt_dir = gt_dir
        self.s_dir = s_dir
        ...
        for i in self.bkgrd_color:
            ...
            self.make_dir(tmp_dir)
            if i == 0:
                self.make_dir(tmp_dir.replace('format_0', 'format_background'))
                self.make_dir(tmp_dir.replace('format_0', 'origin_background'))
        print(msg)
    ...
    def get_letter(self, img, gt_img):
        ...
        if save_0 and save_255:
            ...
            format_name = origin_name_0.replace('origin_0', 'format_background')
            format_letter_0 = cv2.resize(letter_0, (self.format_size[0], self.format_size[1]), interpolation=cv2.INTER_NEAREST)
            format_letter = cv2.resize(origin_img, (self.format_size[0], self.format_size[1]),
